...
const trans = function () {
  ...
  try {
    if (type === 'ascii') {
      const buf = apgConv.Buffer.from(ELEMENT.input.value);

      /* decode ESCAPED format */
      const chars = apgConv.decode('ESCAPED', buf);
      html = '<h1>ASCII</h1>\n';
    ...
    if (type === 'cherokee') {
      ...
      const chars = apgConv.decode('BASE64:UTF8', buf);
      html = '<h1>CHEROKEE</h1>\n';
      html += charsToHtml(chars);
    }
    if (type === 'cards') {
    ...
    if (type === 'base64') {
      const lines = ELEMENT.input.value.split('\n');
      let text = '';
      for (let i = 0; i < 8; i += 1) {
        text += `${lines[i]}<br>`;
      }
