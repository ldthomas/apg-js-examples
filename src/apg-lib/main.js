...
module.exports = function main(args) {
  ...
  try {
    ...
    const obj = require('./phone-number');
    const grammar = new obj();
    ...
    if (doStats) {
      ...
      const name = `${dir}/simple-stats.html`;
      try {
        fs.mkdirSync(dir);
      } catch (e) {
        if (e.code !== 'EEXIST') {
    ...
    if (doTrace) {
      ...
      const html = parser.trace.toHtmlPage('ascii', 'good phone number, default trace');
      const name = `${dir}/simple-trace.html`;
      try {
        fs.mkdirSync(dir);
      } catch (e) {
        if (e.code !== 'EEXIST') {
