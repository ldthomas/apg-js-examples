...
(function () {
    ...
    let htmlWrite = require("../writeHtml.js");
    let fileName = "more-evaluate-rule";
    let id = apgLib.ids;
    let setup = require("./more-setup.js");
    // This `UDT` callback will do phrase matching with a call to `evaluateRule()`.
    let evalRuleCallback = function (result, chars, phraseIndex, data) {
    ...
    let udtCallback = function (result, chars, phraseIndex, data) {
        let matchFound = false;
        let phrase = phraseIndex;
        let length = 0;
        while (phrase + 5 <= chars.length) {
